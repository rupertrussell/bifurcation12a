...
int bifurcationWidth = 1000; // was (8192/4)*8;
int bifurcationHeight = 1000; // was (128/4)*8;
float bifurcationPowers[];
int specificLine = 0;
int count, oldCount;

void setup() {
  size(1000,1000); // was size(1620,1024);
  colorMode(HSB);
...
void draw() {
  ...
  for (int j = 0; j < bifurcationHeight; j++) {
    ...
    for (int i = 0; i < bifurcationWidth; i++) {
      point(10+map(i, 0, bifurcationWidth, 0, bifurcationWidth * 1), height*(1-(float)iteration[i][j]));

      // The most interesting behaviour is found between 3 & 4
      float x = map(i, 0, bifurcationWidth, 1, 5);
